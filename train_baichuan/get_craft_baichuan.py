...
def construct_rait_data(Dsrc, knowledge_states, knowledge_flow, tau_mu, N_van, N_idk):
    ...
    for xi in D_rait:
        ...
        first_sentence = text[:period_index + 1]  # Include the period at the end of the first sentence
        remaining_text = text[period_index + 1:].strip()
        idk_instruction = "If you do not know the answer, please choose E"

        xi_new = {"instruction": first_sentence + " " + idk_instruction, 
                  "input": remaining_text, 
                  "output": xi['text']['output']
                }
        D_rait_new.append(xi_new)
    
    return D_rait_new

# Step 3: Read in the JSON files for the original and perturbed models
def read_files(original_file, perturbed_file):
    """
    ...
    """
    # Load the original data from a pickle file
    with open(original_file, 'rb') as f:
        original_data = pickle.load(f)

    # Load the perturbed data from a pickle file
...
def main():
    ...
    original_file = './train_baichuan/MMLU_craft_musigma.pkl'
    perturbed_file = './train_baichuan/MMLU_reh_craft_musigma.pkl'

    # Step 1: Read in the files
    original_states, perturbed_states = read_files(original_file, perturbed_file)
    ...
    tau_mu = 0.5 # 0.99  # Threshold for correctness
    N_van = 2000  # Number of vanilla samples
    N_idk = 300  # Number of IdK (I don’t know) samples

    D_rait_new = construct_rait_data(Dsrc, original_states, knowledge_flow, tau_mu, N_van, N_idk)

    # LMFlow_data = {"type":"text_only","instances":[]}
    import random
    random.shuffle(D_rait_new)

    with open(f"train_baichuan/rehearsal_train_llamafactory/baichuan_craft_dataset_llama.json",'w') as f:
        json.dump(D_rait_new, f, indent=4)
