...
def gen_prompt(input_list,subject,prompt_data):
    ...
    # TODO: Add a prompt to ask the user to choose E if they don't know the answer
    prompt += "If you do not know the answer, please choose E. "
...
def inference(tokenizer,model,input_text,subject,prompt_data):
    ...
    output_text = {0: "A", 1: "B", 2: "C", 3: "D", 4: "E"}[np.argmax(probs)]
    conf = np.max(probs)
...
if __name__ == "__main__":
    ...
    with open(f"./res/{args.result}_{args.domain}_{args.output_dir}_raw.json",'w') as f:
